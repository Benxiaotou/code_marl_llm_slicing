...
class CustomEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        for i in range(self.num_nodes):
            self.DG.nodes[i]['comp_capacity'] = 1000
            self.DG.nodes[i]['cach_capacity'] = 500
        ...
        for i in range(self.num_links):
            ...
            self.DG[s][d]['bandwidth'] = 33  # Gbps
            self.DG[s][d]['allocated_bandw'] = 0  # 物理网络链路中已分配的带宽资源
            self.link_capacities = 33  # Gbps #重新修改初始的链路容量参数
        ...
        ###************############# 构造包含k_net和g_net数据集
        ##**# re_n-8
        remove_nodes_set = [[0, 1, 2, 3, 11, 12, 14, 16], # 38
                            [3, 5, 9, 10, 14, 16, 18, 21], # 40
                            [0, 1, 2, 4, 8, 11, 14, 16], # 44
                            [2, 4, 6, 8, 11, 14, 19, 22], # 46
                            [0, 2, 4, 8, 11, 12, 14, 16], # 40
                            [0, 1, 2, 11, 14, 16, 17, 22], # 44
                            [0, 2, 3, 5, 11, 14, 16, 21], # 42
                            [0, 3, 4, 8, 11, 14, 16, 18], # 44
                            [0, 1, 2, 3, 7, 11, 14, 16], # 40
                            [0, 2, 3, 11, 14, 16, 17, 22]] # 44
        re_n_set_num_nodes = 8  # ！！移出的节点数量
        re_n_set_num_links = [38, 40, 44, 46, 40, 44, 42, 44, 40, 44] # ！各个net移出的链路数量

        # ##**# re_n-10
        # remove_nodes_set = [[0, 3, 4, 8, 11, 16, 17, 19, 20, 22], # 38
        #                     [0, 1, 2, 3, 4, 5, 8, 11, 14, 16], # 36
        #                     [0, 4, 8, 11, 14, 16, 17, 18, 19, 22], # 40
        #                     [0, 2, 4, 8, 11, 14, 16, 17, 19, 22], # 40
        #                     [0, 1, 2, 3, 4, 8, 11, 14, 16, 18], # 36
        #                     [3, 4, 6, 8, 11, 15, 17, 19, 20, 22], # 38
        #                     [0, 1, 2, 3, 11, 14, 16, 17, 20, 22], # 36
        #                     [0, 1, 2, 3, 6, 10, 11, 14, 16, 18], # 32
        #                     [0, 3, 4, 6, 8, 11, 16, 17, 19, 22], # 40
        #                     [1, 2, 3, 4, 5, 8, 14, 16, 19, 22]] # 38
        # re_n_set_num_nodes = 10  # ！！移出的节点数量
        # re_n_set_num_links = [38, 36, 40, 40, 36, 38, 36, 32, 40, 38] # ！各个net移出的链路数量

        # ##**# re_n-14
        # remove_nodes_set = [[0, 1, 2, 3, 4, 7, 8, 11, 14, 16, 17, 18, 19, 22], # 22
        #                     [0, 1, 2, 3, 5, 6, 11, 12, 13, 14, 16, 17, 21, 22], # 22
        #                     [0, 1, 2, 4, 5, 6, 8, 11, 14, 16, 17, 18, 21, 22], # 22
        #                     [0, 1, 2, 3, 4, 5, 6, 11, 12, 13, 14, 16, 18, 21], # 20
        #                     [2, 4, 5, 6, 8, 9, 11, 14, 16, 17, 18, 19, 21, 22], # 24
        #                     [3, 4, 6, 7, 8, 9, 10, 11, 15, 16, 17, 18, 19, 22], # 24
        #                     [1, 2, 3, 4, 6, 8, 11, 14, 15, 16, 17, 19, 20, 22], # 24
        #                     [0, 1, 2, 4, 5, 8, 11, 12, 14, 16, 17, 19, 21, 22], # 22
        #                     [0, 1, 2, 3, 5, 11, 12, 13, 14, 16, 17, 20, 21, 22], # 22
        #                     [0, 2, 3, 4, 6, 7, 8, 11, 14, 15, 16, 17, 19, 22]] # 24
        # re_n_set_num_nodes = 14  # ！！移出的节点数量
        # re_n_set_num_links = [22, 22, 22, 20, 24, 24, 24, 22, 22, 24] # ！各个net移出的链路数量

        # ##**# re_n-16
        # remove_nodes_set = [[0, 1, 2, 3, 4, 5, 6, 8, 11, 14, 16, 17, 18, 19, 21, 22], # 16
        #                     [0, 1, 2, 3, 4, 5, 8, 9, 10, 11, 14, 16, 17, 18, 21, 22], # 14
        #                     [0, 3, 4, 5, 6, 7, 8, 11, 13, 14, 15, 16, 17, 19, 21, 22], # 16
        #                     [0, 1, 2, 3, 4, 5, 6, 8, 11, 12, 13, 14, 16, 17, 20, 22], # 16
        #                     [0, 1, 2, 3, 4, 5, 7, 8, 11, 14, 16, 17, 18, 19, 20, 22], # 14
        #                     [0, 1, 2, 4, 6, 7, 8, 10, 11, 14, 15, 16, 17, 18, 19, 22], # 16
        #                     [0, 1, 2, 3, 4, 5, 6, 8, 11, 14, 15, 16, 17, 18, 19, 22], # 16
        #                     [0, 1, 2, 3, 4, 5, 8, 10, 11, 14, 16, 17, 18, 19, 20, 22], # 16
        #                     [0, 1, 2, 3, 4, 5, 6, 8, 11, 13, 14, 15, 16, 17, 19, 22], # 18
        #                     [0, 1, 2, 3, 4, 5, 6, 8, 11, 13, 14, 15, 16, 19, 21, 22]]# 16
        # re_n_set_num_nodes = 16  # ！！移出的节点数量
        # re_n_set_num_links = [16, 14, 16, 16, 14, 16, 16, 16, 18, 16] # ！各个net移出的链路数量
        ...
        self.observation_space = spaces.Box(low=0, high=1.0, shape=(3 * obs_k_one_shape + obs_g_one_shape,), dtype=np.float32)
        ...
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(3*self.action_k_one_shape + self.action_k_one_shape,), dtype=np.float32)
        ...
        self.state = np.hstack((obs_k1, obs_k2, obs_k3, obs_g))
    ...
    def step(self, action):  # 输出层使用 tanh，即输出为 -1～1 之间的浮点数; 经过缩放，对应到实际动作
        ...
        action_dict['agent_g'] = action[self.action_k_one_shape*3:]
        ...
        reward = (reward_k1 + reward_k2 + reward_k3 + reward_g) / 4
...
def model_eval(env, model):
    ...
    while episode_count < n_episodes:
        ...
        if eval_step_count % env.eval_max_step == 0:
            episode_count += 1
            with open(env.config.path_reward_save + '/' + env.config.save_attach_word + 'eval_marl_env.pkl',
                      'rb') as handle:
            ...
            os.remove(env.config.path_reward_save + '/' + env.config.save_attach_word + 'eval_marl_env.pkl')
        else:
            pass

    # with open(env.config.path_reward_save + '/' + env.config.save_attach_word + 'dict_train_index_marl_env.pkl',
    #           'rb') as handle:
...
if __name__ == "__main__":
    ...
    save_attach_word = str(configs_dict['max_num_steps']) + '_' \
                       + configs_dict['topology_file'] + '_SingleRL_rn-8' + '_' # ！！

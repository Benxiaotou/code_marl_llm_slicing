...
class ReGEANTTopology(object):

    def __init__(self, remove_nodes_idx, config, data_dir, attach_word):
        ...
        ###*********#
        self.load_topology()#! 如果使用self.node_based_selection()或者self.node_idx_to_id, 则使用该代码
        self.DG = nx.DiGraph()#! 如果使用self.node_based_selection()或者self.node_idx_to_id, 则使用该代码
        re_nodes_idx_list, re_nodes_degree_list = self.node_based_selection()
        ### [7, 17, 21, 19, 18, 2, 16, 0, 11, 1, 8, 15, 6, 10, 5, 9, 4, 20, 3, 13, 14, 12, 22]
        ### [3.0, 3.0, 3.0, 3.0, 2.0, 3.0, 2.0, 3.0, 2.0, 4.0, 2.0, 5.0, 2.0, 5.0, 2.0, 5.0, 2.0, 5.0, 2.0, 6.0, 2.0, 6.0, 2.0]
        ### degree= array([3., 4., 3., 2., 2., 2., 2., 3., 2., 5., 5., 2., 6., 6., 2., 5., 2., 3., 2., 3., 5., 3., 2.])
        ### degree为2: [3,4,5,6,8,11,14,16,18,22]

        self.remove_nodes_idx = remove_nodes_idx#节点索引idx
        self.remove_nodes_id = []#节点标签id
        for i_r in range(len(self.remove_nodes_idx)):
            self.remove_nodes_id.append(self.node_idx_to_id[self.remove_nodes_idx[i_r]])
        print('remove_nodes_idx = ' + str(self.remove_nodes_idx) + '; remove_nodes_id = ' + str(self.remove_nodes_id))

        # self.load_topology()
        # DiGraph.remove_nodes_from(nodes)[源代码]##https://www.osgeo.cn/networkx/reference/classes/generated/networkx.DiGraph.remove_nodes_from.html#networkx.DiGraph.remove_nodes_from
        # self.DG.remove_nodes_from(self.remove_nodes_idx)

        self.re_attach_word = attach_word# + "_re_" + "_".join(str(x) for x in self.remove_nodes_idx)
        self.topology_file = data_dir + config.topology_file + '/' + config.topology_file + self.re_attach_word #
        self.shortest_paths_file = self.topology_file + '_shortest_paths'

        self.re_generate_topology_file()
        ##*********#
            ...
            with open(self.topology_file, 'w') as f:
                ...
                for idx in range(num_links):
                    f.write("{:d}\t{:d}\t{:d}\t1\t10000000\t('{:s}', '{:s}')\n".format(idx,
                                                                                       self.link_idx_sort_list[idx][0],
                                                                                       self.link_idx_sort_list[idx][1],
                                                                                       self.node_id_list[self.link_idx_sort_list[idx][0]],
                                                                                       self.node_id_list[self.link_idx_sort_list[idx][1]]))

    def re_generate_topology_file(self):
        ...
        for node in root[1][0].iter(tag='node'):  # iter(tag=None) 遍历Element的child，可以指定tag精确查找
            node_id = node.get(key='id')  # get(key, default=None) 获取元素指定key对应的attrib，如果没有attrib，返回default。
            if node_id in self.remove_nodes_id:
                print('re_node_id=' + node_id)
                continue
            self.node_id_list.append(node_id)
            self.node_id_to_idx[node_id] = index
            index += 1
        ...
        for link in root[1][1].iter(tag='link'):  # iter(tag=None) 遍历Element的child，可以指定tag精确查找
            from_node_id = link[0].get(key='node')
            to_node_id = link[1].get(key='node')
            if from_node_id in self.remove_nodes_id or to_node_id in self.remove_nodes_id:
                print('re_link_id=' + from_node_id+'__'+to_node_id)
                continue
            self.link_id_list.append((from_node_id, to_node_id))
            self.link_idx_list.append((self.node_id_to_idx[from_node_id], self.node_id_to_idx[to_node_id]))
            ...
            for s in range(self.num_nodes):
                for d in range(self.num_nodes):
                    if s != d:
                        ...
                        self.num_pairs += 1
                        self.shortest_paths.append(
                            list(nx.all_shortest_paths(self.DG, s, d, weight='weight', method='dijkstra'))[:MAX_PATH_NUM]
                        )
                        line = str(s) + '->' + str(d) + ': ' + str(self.shortest_paths[-1])
                        f.writelines(line + '\n')
    ...
    def node_based_selection(self):
        ...
        # print('The degree of nodes is '+ str(degree))
        # 从中位数开始往大和往小依次交错排序
        ...
        re_nodes_idx_list = []
        re_nodes_degree_list = []

        for i_mk in idx_median_list:
            idx_key = idx_degree_sort[i_mk]  # 将交错排序后的索引置换成idx_degree_sort的索引
            re_nodes_idx_list.append(idx_key)
            re_nodes_degree_list.append(degree[idx_key])
        ...
        return re_nodes_idx_list, re_nodes_degree_list


class ReGEANTTraffic(object):

    def __init__(self, config, topology, data_dir, is_training=True):
        if is_training:
            self.traffic_file = data_dir + config.topology_file + '/' + config.traffic_file + topology.re_attach_word #
        else:
            self.traffic_file = data_dir + config.topology_file + '/' + config.test_traffic_file + topology.re_attach_word #
        self.num_nodes = topology.num_nodes
        self.load_traffic(topology, config, is_training)
    ...
    def read_traffic(self, topology, config, is_training):  ##
        ...
        with open(self.traffic_file, 'w') as f:
            ...
            for file_name in sorted_files:
                ...
                for src in root[1].iter(tag='src'):  # iter(tag=None) 遍历Element的child，可以指定tag精确查找
                    src_id = src.get(key='id')  # get(key, default=None) 获取元素指定key对应的attrib，如果没有attrib，返回default。
                    if src_id in topology.remove_nodes_id:
                        # print('src_id=' + src_id)
                        continue
                    s = topology.node_id_to_idx[src_id]
                    for dst in src.iter(tag='dst'):
                        dst_id = dst.get(key='id')
                        if dst_id in topology.remove_nodes_id:
                            # print('dst_id=' + dst_id)
                            continue
                        d = topology.node_id_to_idx[dst_id]
                        demand_value = dst.text

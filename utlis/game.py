...
# def __init__(env, config, env, random_seed=1000):
#     env.random_state = np.random.RandomState(seed=random_seed)
# 
#     env.data_dir = env.data_dir
#     env.DG = env.topology.DG
#     env.traffic_file = env.traffic_file
#     env.traffic_matrices = env.traffic_matrices
#     env.traffic_matrices_dims = env.traffic_matrices.shape
#     env.tm_cnt = env.tm_cnt
#     env.num_pairs = env.num_pairs
#     env.pair_idx_to_sd = env.pair_idx_to_sd
#     env.pair_sd_to_idx = env.pair_sd_to_idx
#     env.num_nodes = env.num_nodes
#     env.num_links = env.num_links
#     env.link_idx_to_sd = env.link_idx_to_sd
#     env.link_sd_to_idx = env.link_sd_to_idx
#     env.link_capacities = env.link_capacities
#     env.link_weights = env.link_weights
#     env.shortest_paths_node = env.shortest_paths_node  # paths with node info
#     env.shortest_paths_link = env.shortest_paths_link  # paths with link info
# 
#     env.get_ecmp_next_hops()
# 
#     env.model_type = config.model_type
# 
#     # for LP
#     env.lp_pairs = [p for p in range(env.num_pairs)]
#     env.lp_nodes = [n for n in range(env.num_nodes)]
#     env.links = [e for e in range(env.num_links)]
#     env.lp_links = [e for e in env.link_sd_to_idx]
#     env.pair_links = [(pr, e[0], e[1]) for pr in env.lp_pairs for e in env.lp_links]
# 
#     env.load_multiplier = {}

def generate_inputs(env, normalization=True):
    env.normalized_traffic_matrices = np.zeros(
        (env.valid_tm_cnt, env.traffic_matrices_dims[1], env.traffic_matrices_dims[2], env.tm_history),
        dtype=np.float32)  # tm state  [Valid_tms, Node, Node, History]
    idx_offset = env.tm_history - 1
    for tm_idx in env.tm_indexes:
        for h in range(env.tm_history):
            if normalization:
                tm_max_element = np.max(env.traffic_matrices[tm_idx - h])
                env.normalized_traffic_matrices[tm_idx - idx_offset, :, :, h] = env.traffic_matrices[
                                                                                     tm_idx - h] / tm_max_element  # [Valid_tms, Node, Node, History]
            else:
                env.normalized_traffic_matrices[tm_idx - idx_offset, :, :, h] = env.traffic_matrices[
                    tm_idx - h]  # [Valid_tms, Node, Node, History]

def get_topK_flows(env, tm_idx, pairs):
    tm = env.traffic_matrices[tm_idx]
    f = {}
    for p in pairs:
        s, d = env.pair_idx_to_sd[p]
        f[p] = tm[s][d]

    sorted_f = sorted(f.items(), key=lambda kv: (kv[1], kv[0]), reverse=True)

    cf = []
    for i in range(env.max_moves):
        cf.append(sorted_f[i][0])

    return cf

def get_ecmp_next_hops(env):
    env.ecmp_next_hops = {}
    for src in range(env.num_nodes):
        for dst in range(env.num_nodes):
            if src == dst:
                continue
            env.ecmp_next_hops[src, dst] = []
            for p in env.shortest_paths_node[env.pair_sd_to_idx[(src, dst)]]:
                if p[1] not in env.ecmp_next_hops[src, dst]:
                    env.ecmp_next_hops[src, dst].append(p[1])

def ecmp_next_hop_distribution(env, link_loads, demand, src, dst):
    if src == dst:
        return

    ecmp_next_hops = env.ecmp_next_hops[src, dst]

    next_hops_cnt = len(ecmp_next_hops)
    # if next_hops_cnt > 1:
    # print(env.shortest_paths_node[env.pair_sd_to_idx[(src, dst)]])

    ecmp_demand = demand / next_hops_cnt
    for np in ecmp_next_hops:
        link_loads[env.link_sd_to_idx[(src, np)]] += ecmp_demand
        env.ecmp_next_hop_distribution(link_loads, ecmp_demand, np, dst)

def ecmp_traffic_distribution(env, tm_idx):
    link_loads = np.zeros((env.num_links))
    tm = env.traffic_matrices[tm_idx]
    for pair_idx in range(env.num_pairs):
        s, d = env.pair_idx_to_sd[pair_idx]
        demand = tm[s][d]
        if demand != 0:
            env.ecmp_next_hop_distribution(link_loads, demand, s, d)

    return link_loads

def get_critical_topK_flows(env, tm_idx, critical_links=5):
    link_loads = env.ecmp_traffic_distribution(tm_idx)
    critical_link_indexes = np.argsort(-(link_loads / env.link_capacities))[:critical_links]

    cf_potential = []
    for pair_idx in range(env.num_pairs):
        for path in env.shortest_paths_link[pair_idx]:
            if len(set(path).intersection(critical_link_indexes)) > 0:
                cf_potential.append(pair_idx)
                break

    # print(cf_potential)
    assert len(cf_potential) >= env.max_moves, \
        ("cf_potential(%d) < max_move(%d), please increse critical_links(%d)" % (
        cf_potential, env.max_moves, critical_links))

    return env.get_topK_flows(tm_idx, cf_potential)

def eval_ecmp_traffic_distribution(env, tm_idx, eval_delay=False):
    eval_link_loads = env.ecmp_traffic_distribution(tm_idx)
    eval_max_utilization = np.max(eval_link_loads / env.link_capacities)
    env.load_multiplier[tm_idx] = 0.9 / eval_max_utilization
    delay = 0
    if eval_delay:
        eval_link_loads *= env.load_multiplier[tm_idx]
        delay = sum(eval_link_loads / (env.link_capacities - eval_link_loads))

    return eval_max_utilization, delay

def optimal_routing_mlu(env, tm_idx):
    tm = env.traffic_matrices[tm_idx]
    demands = {}
    for i in range(env.num_pairs):
        s, d = env.pair_idx_to_sd[i]
        demands[i] = tm[s][d]

    model = LpProblem(name="routing")

    ratio = LpVariable.dicts(name="ratio", indexs=env.pair_links, lowBound=0, upBound=1)

    link_load = LpVariable.dicts(name="link_load", indexs=env.links)

    r = LpVariable(name="congestion_ratio")

    for pr in env.lp_pairs:
        model += (
        lpSum([ratio[pr, e[0], e[1]] for e in env.lp_links if e[1] == env.pair_idx_to_sd[pr][0]]) - lpSum(
            [ratio[pr, e[0], e[1]] for e in env.lp_links if e[0] == env.pair_idx_to_sd[pr][0]]) == -1,
        "flow_conservation_constr1_%d" % pr)

    for pr in env.lp_pairs:
        model += (
        lpSum([ratio[pr, e[0], e[1]] for e in env.lp_links if e[1] == env.pair_idx_to_sd[pr][1]]) - lpSum(
            [ratio[pr, e[0], e[1]] for e in env.lp_links if e[0] == env.pair_idx_to_sd[pr][1]]) == 1,
        "flow_conservation_constr2_%d" % pr)

    for pr in env.lp_pairs:
        for n in env.lp_nodes:
            if n not in env.pair_idx_to_sd[pr]:
                model += (lpSum([ratio[pr, e[0], e[1]] for e in env.lp_links if e[1] == n]) - lpSum(
                    [ratio[pr, e[0], e[1]] for e in env.lp_links if e[0] == n]) == 0,
                          "flow_conservation_constr3_%d_%d" % (pr, n))

    for e in env.lp_links:
        ei = env.link_sd_to_idx[e]
        model += (link_load[ei] == lpSum([demands[pr] * ratio[pr, e[0], e[1]] for pr in env.lp_pairs]),
                  "link_load_constr%d" % ei)
        model += (link_load[ei] <= env.link_capacities[ei] * r, "congestion_ratio_constr%d" % ei)

    model += r + OBJ_EPSILON * lpSum([link_load[e] for e in env.links])
...
def eval_optimal_routing_mlu(env, tm_idx, solution, eval_delay=False):
    optimal_link_loads = np.zeros((env.num_links))
    eval_tm = env.traffic_matrices[tm_idx]
    for i in range(env.num_pairs):
        s, d = env.pair_idx_to_sd[i]
        demand = eval_tm[s][d]
        for e in env.lp_links:
            link_idx = env.link_sd_to_idx[e]
            optimal_link_loads[link_idx] += demand * solution[i, e[0], e[1]]

    optimal_max_utilization = np.max(optimal_link_loads / env.link_capacities)
    delay = 0
    if eval_delay:
        assert tm_idx in env.load_multiplier, (tm_idx)
        optimal_link_loads *= env.load_multiplier[tm_idx]
        delay = sum(optimal_link_loads / (env.link_capacities - optimal_link_loads))

    return optimal_max_utilization, delay

def optimal_routing_mlu_critical_pairs(env, tm_idx, critical_pairs):
    tm = env.traffic_matrices[tm_idx]

    pairs = critical_pairs

    demands = {}
    background_link_loads = np.zeros((env.num_links))
    for i in range(env.num_pairs):
        s, d = env.pair_idx_to_sd[i]
        # background link load
        if i not in critical_pairs:
            env.ecmp_next_hop_distribution(background_link_loads, tm[s][d], s, d)
        else:
            demands[i] = tm[s][d]

    model = LpProblem(name="routing")  ##LpProblem() 构造函数还有一个可选参数 sense，用于指定问题的类型。如果省略了这个参数，问题的类型默认为 LpMinimize。

    pair_links = [(pr, e[0], e[1]) for pr in pairs for e in env.lp_links]
    ...
    model += r + OBJ_EPSILON * lpSum([link_load[ei] for ei in env.links])
...
def eval_critical_flow_and_ecmp(env, tm_idx, critical_pairs, solution, eval_delay=False):
    eval_tm = env.traffic_matrices[tm_idx]
    eval_link_loads = np.zeros((env.num_links))
    for i in range(env.num_pairs):
        s, d = env.pair_idx_to_sd[i]
        if i not in critical_pairs:
            env.ecmp_next_hop_distribution(eval_link_loads, eval_tm[s][d], s, d)
        else:
            demand = eval_tm[s][d]
            for e in env.lp_links:
                link_idx = env.link_sd_to_idx[e]
...
def optimal_routing_delay(env, tm_idx):
    assert tm_idx in env.load_multiplier, (tm_idx)
    tm = env.traffic_matrices[tm_idx] * env.load_multiplier[tm_idx]
    demands = {}
    for i in range(env.num_pairs):
        s, d = env.pair_idx_to_sd[i]
        demands[i] = tm[s][d]
    ...
    f = LpVariable.dicts(name="link_cost", indexs=env.links)
    ...
    for e in env.lp_links:
        ...
        model += (link_load[ei] == lpSum([demands[pr] * ratio[pr, e[0], e[1]] for pr in env.lp_pairs]),
                  "link_load_constr%d" % ei)
        model += (f[ei] * env.link_capacities[ei] >= link_load[ei], "cost_constr1_%d" % ei)
        model += (f[ei] >= 3 * link_load[ei] / env.link_capacities[ei] - 2 / 3, "cost_constr2_%d" % ei)
        model += (f[ei] >= 10 * link_load[ei] / env.link_capacities[ei] - 16 / 3, "cost_constr3_%d" % ei)
        model += (f[ei] >= 70 * link_load[ei] / env.link_capacities[ei] - 178 / 3, "cost_constr4_%d" % ei)
        model += (f[ei] >= 500 * link_load[ei] / env.link_capacities[ei] - 1468 / 3, "cost_constr5_%d" % ei)
        model += (f[ei] >= 5000 * link_load[ei] / env.link_capacities[ei] - 16318 / 3, "cost_constr6_%d" % ei)

    model += lpSum(f[ei] for ei in env.links)
...
def eval_optimal_routing_delay(env, tm_idx, solution):
    optimal_link_loads = np.zeros((env.num_links))
    assert tm_idx in env.load_multiplier, (tm_idx)
    eval_tm = env.traffic_matrices[tm_idx] * env.load_multiplier[tm_idx]
    for i in range(env.num_pairs):
        s, d = env.pair_idx_to_sd[i]
        demand = eval_tm[s][d]
        for e in env.lp_links:
    ...
    optimal_delay = sum(optimal_link_loads / (env.link_capacities - optimal_link_loads))

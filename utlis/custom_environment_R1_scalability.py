...
#*r1sc*#
from utlis.network_env_r1sc import AbileneTopology, AbileneTraffic, \
...
class CustomEnv(gym.Env):
    # *r1sc*#
    def __init__(self, path_reward_save, link_num_ratio, flow_num_ratio, RemoveNode, is_training=True):
        ...
        if config.topology_file == 'Abilene':  # *#
            ...
            # *r1sc*#
            self.topology = GEANTTopology(RemoveNode, config, self.data_dir)
        ...
        if config.method == 'pure_policy':
            self.baseline = {}
        # *r1sc*#
        attach_word = '_rmnd_' + '_'.join(map(str, RemoveNode)) + '_'
        self.ecmp_mlu_links_util_save_file = self.data_dir + self.topology_file + '/' + self.topology_file + attach_word + '_ecmp_mlu_links_util_save.pkl'
        ...
        # *r1sc*#
        attach_word = '_rmnd_' + '_'.join(map(str, RemoveNode)) + '_'
        self.optimal_mlu_save_file = self.data_dir + self.topology_file + '/' + self.topology_file + attach_word + '_optimal_mlu_save.pkl'
        ...
        self.observation_space = spaces.Box(low=0, high=1.0, shape=(3, self.num_nodes, self.num_nodes),
                                            dtype=np.float32)
    ...
    def reset(self):
        ...
        # self.idx_crit_links = self._centrality_based_selection()
    ...
    def _eval_crit_links_and_crit_flows(self, tm_idx, action_CL):  # action_CL中的值范围[0,1]
        ...
        # idx_crit_flows = self.idx_crit_flows  # Crit K
        # len_crit_flows = len(idx_crit_flows)
        # vrate_crit_flows = np.ones(len_crit_flows)
        # vrate_crit_flows[:math.ceil(len_crit_flows * 0.3)] = action_CL[-3]
        # vrate_crit_flows[math.ceil(len_crit_flows * 0.3):math.ceil(len_crit_flows * 0.6)] = action_CL[-2]
        # vrate_crit_flows[math.ceil(len_crit_flows * 0.6):] = action_CL[-1]

        # 使用全部的真实link权重所得到的默认多路径路由，对非关键流进行流量路由分配, ##并对关键流的部分流量进行路由
        for idx_DF in range(self.num_pairs):
            ...
            # else:#关键流的部分流量
            #     s, d = self.pair_idx_to_sd[idx_DF]
            #     demand = eval_tm[s][d] * (1 - vrate_crit_flows[idx_crit_flows.index(idx_DF)])
            #     #demand = eval_tm[s][d] * (1 - action_CL[-1])##action_CL[-1]代表关键流的流量虚拟比例
            #     if demand != 0:
            #         self._ecmp_next_hop_distribution(eval_link_loads, demand, s, d)
        ...
        #                 if counter == K_num - 1:
        #                     break
        #             virtual_shortest_paths_node.append(KSP_list)
        #
        #             # line = str(s) + '->' + str(d) + ': ' + str(virtual_shortest_paths_node[-1])
        #             # f.writelines(line + '\n')

        K_num = 3  # **# 3 #Abilene：1，GEANT：3，Xspedius：3，SwitchL3：3
        ...
        for idx_CF in idx_crit_flows:
            ...
            if eval_tm[s][d] != 0:
                ...
                for k in range(
                        K_num - 1):  # 疑问：如果nx.shortest_path得到的path为多个，则最终循环后的path数量超过K_num？ 答：该函数应该仅仅返回一个路径（即使是等价多路径，也是返回一个），参见：https://www.osgeo.cn/networkx/reference/algorithms/generated/networkx.algorithms.shortest_paths.generic.shortest_path.html?highlight=shortest_path
        ...
        for idx_CF in idx_crit_flows:
            s, d = self.pair_idx_to_sd[idx_CF]
            demand = eval_tm[s][d]
            # demand = eval_tm[s][d] * vrate_crit_flows[idx_crit_flows.index(idx_CF)]
            # demand = eval_tm[s][d] * (1 - action_CL[-1])##action_CL[-1]代表关键流的流量虚拟比例
            if demand != 0:
                # self._multipath_next_hop_distribution(eval_link_loads, demand, s, d,

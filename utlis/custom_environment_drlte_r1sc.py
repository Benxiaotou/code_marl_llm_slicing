...
class CustomEnv(gym.Env):
    ...
    def __init__(self, path_reward_save, link_num_ratio, flow_num_ratio, RemoveNode, is_training=True):
        ...
        for idx_CF in range(self.num_pairs):
            ...
            self.drlte_K_shortest_paths[idx_CF] = paths_list

            # ##（法2）**#生成K短路径#当Abilene时应更改K_num
            # shortest_paths_generator = nx.shortest_simple_paths(self.DG, s, d, weight='weight')
            # KSP_list = []
            # for counter, path in enumerate(shortest_paths_generator):
            #     KSP_list.append(path)
            #     if counter == K_num - 1:
            #         break
            # self.drlte_K_shortest_paths[idx_CF] = KSP_list


        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(3*self.num_pairs + 0,), dtype=np.float32)
    ...
    def _eval_crit_links_and_crit_flows(self, tm_idx, action_CL):  # action_CL中的值范围[0,1]
        ...
        assert len(action_CL) == 3*self.num_pairs, "Error from action_CL"
        for idx_CF in range(self.num_pairs):
            s, d = self.pair_idx_to_sd[idx_CF]
            demand = eval_tm[s][d]
            if demand != 0:
                paths = self.drlte_K_shortest_paths[idx_CF]
                action_s = action_CL[idx_CF*3:idx_CF*3+3]
                action_s[len(paths):] = 0
                action_s[:len(paths)] = action_s[:len(paths)]/np.sum(action_s[:len(paths)]) \
                    if np.sum(action_s[:len(paths)]) != 0 else np.ones(len(paths))/len(paths)
                self._ksp_next_hop_distribution(action_s, eval_link_loads, demand, s, d, paths)
    ...
    def _ksp_next_hop_distribution(self, action_s, link_loads, demand, src, dst,
                                   paths):
        ...
        # assert action_s.sum() == 1, "Error from action_s"
        traffic_demand = demand * np.ones(len(action_s)) * action_s

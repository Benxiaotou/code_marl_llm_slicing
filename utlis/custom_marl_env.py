...
class CustomMultiAgentEnv(RawMultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        if os.path.exists(self.sample_dict_save_file):
            ...
            for i_s in range(self.sample_cnt):
                ...
                for i_k in range(self.K_num_existing_v_net):
                    ...
                    print("comp_capacity = " + str(comp_capacity) + "; cach_capacity = " + str(cach_capacity) + "; bandwidth = " + str(bandwidth))
        ...
        self.state_space = Box(low=-1.0, high=1.0, shape=(self.K_num_existing_v_net * obs_k_one_shape + obs_g_one_shape,), dtype=np.float32)
        ...
        self.action_space = {self.agents[0]: Box(low=-1, high=1.0, shape=[self.action_k_one_shape, ], dtype=np.float32),
                             ...
                             self.agents[-1]: Box(low=-1, high=1.0, shape=[self.action_g_one_shape, ], dtype=np.float32)}
    ...
    def step(self, action_dict):
        ...
        idx_k1_overlap_nodes, idx_k1_overlap_links, sd_k_overlap_links = self._get_overlap_node_link(k_net=self.k1_net, g_net=self.g_net)
        idx_k1_overlap_nodes = sorted(idx_k1_overlap_nodes)  # ！ 按照从小到大的顺序排列
        idx_k1_overlap_links = sorted(idx_k1_overlap_links)  # ！ 按照从小到大的顺序排列
        ...
        idx_k2_overlap_nodes, idx_k2_overlap_links, sd_k_overlap_links = self._get_overlap_node_link(k_net=self.k2_net, g_net=self.g_net)
        idx_k2_overlap_nodes = sorted(idx_k2_overlap_nodes)  # ！ 按照从小到大的顺序排列
        idx_k2_overlap_links = sorted(idx_k2_overlap_links)  # ！ 按照从小到大的顺序排列
        ...
        idx_k3_overlap_nodes, idx_k3_overlap_links, sd_k_overlap_links = self._get_overlap_node_link(k_net=self.k3_net, g_net=self.g_net)
        idx_k3_overlap_nodes = sorted(idx_k3_overlap_nodes)  # ！ 按照从小到大的顺序排列
        idx_k3_overlap_links = sorted(idx_k3_overlap_links)  # ！ 按照从小到大的顺序排列
        ...
        self.g_net, demand_n_g, demand_l_g, g_done = self._get_g_adjustment(g_net=self.g_net, action_g=action_g,
                                                                            assigned_resoure=assigned_resoure, tm_idx=0)
        if g_done == True:
        # g 的收益计算
        ...
        if self.timestep + 1 == self.max_num_steps:
            with open(self.path_reward_save + '/' + str(self.timestep + 1) + '_' + self.topology_file + '_rewards_marl_env.pkl',
                      'wb') as handle:
                ...
                if used_comp > comp_capacity:  # 计算资源满，则仅弹出计算任务
                    for i_rt in range(len(tasks_info_list)):  # 从tasks_info_list找出排的最靠后的计算任务，并弹出它
                       if tasks_info_list[-(i_rt+1)]['type'] == 'comp':
                           ...
                           tasks_info_list = tasks_info_list[:-(i_rt+1)] + tasks_info_list[-(i_rt+1)+1:] if i_rt != 0 \
            ...
            if type[i] == 0:
                tasks_info_list.append({'type': 'comp',
                                        'dem_comp': np.clip(np.random.normal(loc=(comp_high+comp_low)/2,
                                                                             scale=(comp_high-comp_low)/2), comp_low, comp_high),
                                        'dem_cach': 0.5 * np.clip(np.random.normal(loc=(cach_high+cach_low)/2,
                                                                             scale=(cach_high-cach_low)/2), cach_low, cach_high)})
            else:
                tasks_info_list.append({'type': 'cach',
                                        'dem_comp': 0,
        ...
        for idx_node in range(g_net.num_nodes):
            ...
            if id_node in k_net.node_id_list:
                ...
                if idx_node_k in assigned_comp_k:
                    cd_k_to_g_node_comp[idx_node] = assigned_comp_k[idx_node_k] / g_net.DG.nodes[idx_node]['comp_capacity']
                    cd_k_to_g_node_cach[idx_node] = assigned_cach_k[idx_node_k] / g_net.DG.nodes[idx_node]['cach_capacity']
                else:
    ...
    def _get_contribution_degree(self, assigned_resoure, unassigned_resoure):
        ...
        cd_k1_to_g_node_comp, cd_k1_to_g_node_cach, cd_k1_to_g_link \
            ...
            assigned_comp_k=assigned_comp_k1, assigned_cach_k=assigned_cach_k1, assigned_bandw_k=assigned_bandw_k1)
        cd_k2_to_g_node_comp, cd_k2_to_g_node_cach, cd_k2_to_g_link\
            ...
            assigned_comp_k=assigned_comp_k2, assigned_cach_k=assigned_cach_k2, assigned_bandw_k=assigned_bandw_k2)
        cd_k3_to_g_node_comp, cd_k3_to_g_node_cach, cd_k3_to_g_link\
            ...
            assigned_comp_k=assigned_comp_k3, assigned_cach_k=assigned_cach_k3, assigned_bandw_k=assigned_bandw_k3)
        ...
        cd_vstack_comp = np.vstack((cd_k1_to_g_node_comp, cd_k2_to_g_node_comp, cd_k3_to_g_node_comp, p_n_w_factor * cd_p_to_g_node_comp))  # 堆叠
        ...
        cd_vstack_cach = np.vstack((cd_k1_to_g_node_cach, cd_k2_to_g_node_cach, cd_k3_to_g_node_cach, p_n_w_factor * cd_p_to_g_node_cach))  # 堆叠
        ...
        cd_vstack_link = np.vstack((cd_k1_to_g_link, cd_k2_to_g_link, cd_k3_to_g_link, p_l_w_factor * cd_p_to_g_link))  # 堆叠
    ...
    def _get_g_adjustment(self, g_net, action_g, assigned_resoure, tm_idx=0):
        ...
        for idx in range(g_net.num_nodes):
            ...
            comp_capacity = limit_n_allocate * comp_capacity + (1 - limit_n_allocate) * comp_capacity * (1 - action_g[0])
            cach_capacity = limit_n_allocate * cach_capacity + (1 - limit_n_allocate) * cach_capacity * (1 - action_g[1])
        ...
        for idx_link in range(g_net.num_links):
            ...
            idx_s_p_net = self.node_id_to_idx[id_s]
            idx_d_p_net = self.node_id_to_idx[id_d]
            if self.DG[idx_s_p_net][idx_d_p_net]['bandwidth'] - self.DG[idx_s_p_net][idx_d_p_net]['allocated_bandw'] >= bandwidth:  # 带宽资源是否满足需求
    ...
    def _get_k_node_cost(self, k_net, idx_overlap_nodes, action_k_node):
        ...
        for idx_o_n in idx_overlap_nodes:
            ...
            comp_capacity = limit_n_allocate * comp_capacity + (1 - limit_n_allocate) * comp_capacity * (1 - action_k_node[i_n])
            cach_capacity = limit_n_allocate * cach_capacity + (1 - limit_n_allocate) * cach_capacity * (1 - action_k_node[i_n])
            ...
            for i_t_m in range(len(tasks_mig_list)):
                ...
                if mig_task['type'] == 'comp':
                    ...
                    for i_ngb in range(len(neighbors)):  # 遍历一阶邻居节点
                        ...
                        B_nm_res = k_net.DG[idx_o_n][max_neighbor]['bandwidth'] - k_net.DG[idx_o_n][max_neighbor]['used_bandw']

                        max_neighbor_res_comp = k_net.DG.nodes[max_neighbor]['comp_capacity'] - k_net.DG.nodes[max_neighbor]['used_comp']
                        max_neighbor_res_cach = k_net.DG.nodes[max_neighbor]['cach_capacity'] - k_net.DG.nodes[max_neighbor]['used_cach']

                        # 判断该邻居节点是否满足计算和缓存资源的需求, 同时源节点与迁移的目的节点之间的路径带宽满足B_nm的要求
                        if (max_neighbor_res_comp >= mig_task['dem_comp'] and max_neighbor_res_cach >= mig_task['dem_cach']) \
                                and B_nm_res >= B_nm:
                    ...
                    rho_mig_nm = 0.5 * mig_task['dem_cach'] / 5 + 0.5 * np.clip(np.random.normal(loc=1, scale=0.2), 0.8, 1.2)
                    ...
                    # 与迁移数量流的数据量正相关的基础QoS惩罚因子
                    tau_p_d = 0.2 * (k_net.DG.nodes[d_mig_node]['used_cach'] / k_net.DG.nodes[d_mig_node]['cach_capacity']) \
                              * k_net.DG.nodes[d_mig_node]['num_cach_tasks'] \
                              * np.clip(np.random.normal(loc=1, scale=0.1), 0.8, 1.2)
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="configs_marl.yaml")
    ...
    configs_dict['path_env_save_save'] = path_env_save_save
    ...
    if configs.benchmark:
        ...
        #
        # Agents.train(envs.max_episode_steps)
        Agents.train(configs.max_num_steps)

        # with open(configs.path_reward_save + '/' + configs.topology_file + '_Agents.pkl', 'wb') as handle:
        #     pickle.dump(Agents, handle)
        #
        test_scores = Agents.test(env_fn=None, n_episodes=test_episode)
        Agents.save_model(model_name="best_model.pth")
        best_scores_info = {"mean": np.mean(test_scores),
                            "std": np.std(test_scores),
                            "step": Agents.timestep}

...
class CustomEnv(gymnasium.Env):
    ...
    def __init__(self, path_reward_save, link_num_ratio, flow_num_ratio, is_training=True):
        ...
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(self.num_crit_links + 0,), dtype=np.float64)
        self.observation_space = spaces.Box(low=-1, high=1.0, shape=(3, self.num_nodes, self.num_nodes),
                                            dtype=np.float64)

        # #***MARL***
        # self.state = None
        # self.timestep = 0
        # self.K_num_existing_net = 3 #论文中的K
        # self.possible_agents = ["agent_g"] + ["agent_k" + str(r) for r in range(self.K_num_existing_net)]
    ...
    #
    # # Action space should be defined here. # If your spaces change over time, remove this line (disable caching).
    # @functools.lru_cache(maxsize=None)
    ...
    def _eval_crit_links_and_crit_flows(self, tm_idx, action_CL):  # action_CL中的值范围[0,1]
        ...
        ##**#start_time
        start_time = time.time()
        self.max_moves = math.ceil(0.15 * self.num_pairs)
        traffic_reroute = 0
        flow_reroute = 0
        ##**##
        ...
        self.traffic_r_l.append(traffic_reroute/np.sum(eval_tm))
        self.flow_r_l.append(flow_reroute/self.num_pairs)
        print("traffic_reroute/np.sum(eval_tm) = "+ str(traffic_reroute/np.sum(eval_tm)) +
              "; flow_reroute = "+str(flow_reroute)+"; flow_r = "+str(flow_reroute/self.num_pairs))
        # ##**##

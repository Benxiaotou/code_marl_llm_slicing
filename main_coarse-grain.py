...
class CustomMultiAgentEnv(RawMultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        self.action_k_one_shape = 8 # ！！  # 粗粒度控制节点和链路资源的调整比例
        self.action_k_node_control = 4 # ！！ # 粗粒度控制节点资源的调整比例
        self.action_k_link_control = 4 # ！！ # 粗粒度控制链路资源的调整比例
    ...
    def _get_g_adjustment(self, g_net, action_g, assigned_resoure, tm_idx=0):
        ...
        for idx_link in range(g_net.num_links):
            ...
            if len(action_g) == 1:
                bandwidth = limit_l_allocate * old_bandwidth + (1 - limit_l_allocate) * old_bandwidth * (1 - action_g[0])
            elif len(action_g) == 2:
                bandwidth = limit_l_allocate * old_bandwidth + (1 - limit_l_allocate) * old_bandwidth * (1 - action_g[1])
            else:
...
if __name__ == "__main__":
    ...
    save_attach_word = str(configs_dict['max_num_steps']) + '_' \
                       + configs_dict['topology_file'] + '_cg-4' + '_' # ！！
    ...
    if configs.benchmark:
        ...
        marl_model_dir = 'dir_model_cg_4' # ！！ 主要更改它 # 为避免load错误模型，dir 存放一个.pth的model文件

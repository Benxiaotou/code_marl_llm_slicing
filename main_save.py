...
from utlis.custom_environment_save import CustomEnv, eval_env_step#, env_init_eval#, env_init_real_TM_eval, env_init_simu_TM_eval,
...
def model_train():
    ...
    env = CustomEnv(path_reward_save=path_reward_save,
                    link_num_ratio=0.15,# Abilene：0.15，GEANT：0.15，Xspedius：0.15，SwitchL3：0.15    #math.ceil(0.15 * self.num_links)  # 10%*N
                    flow_num_ratio=0.15,# Abilene：0.20, GEANT：0.15，Xspedius：0.15，SwitchL3     #TopK流的K值#10%*N
                    is_training=True)
...
def model_eval(env, model):
    ...
    for k_episode in range(num_try):
        ...
        df.to_csv(path_plt_data + '/' + 'SwitchL3_tr_fl_r_propXXX.csv', header=True, index=False,
...
if __name__ == "__main__":
    ...
    attach_word = '_'#'_LinkNum_0.15_FlowNum_0.15_'#'_'

...
from utlis.custom_environment_drlte_r1sc import CustomEnv, eval_env_step#, env_init_eval#, env_init_real_TM_eval, env_init_simu_TM_eval,

'''展开 # 展开当前代码块ctrl =  # 彻底展开当前代码块ctrl alt =  # 展开所有代码块ctrl shift +
折叠 # 折叠当前代码块ctrl -  # 彻底折叠当前代码块ctrl alt -  # 折叠所有代码块ctrl shift - '''


def model_train(env_save_pkl, model_save_pkl, link_num_ratio, flow_num_ratio, RemoveNode):
    ...
    ##**
    # env.num_crit_links = int(np.round(link_num_ratio * env.num_links))# Abilene：0.15，GEANT：0.15，Xspedius：0.15，SwitchL3：0.15    #math.ceil(0.15 * self.num_links)  # 10%*N
    # env.max_moves = math.ceil(flow_num_ratio * env.num_pairs)# Abilene：0.20, GEANT：0.15，Xspedius：0.15，SwitchL3     #TopK流的K值#10%*N
...
def model_eval(env, model):
    ...
    for k_episode in range(num_try):
        ...
        L = int(len(norm_mlu_eval) / 7)
        df = pd.DataFrame(
            {'norm_mlu_eval': norm_mlu_eval[L * 3:L * 4], 'norm_ecmp_mlu_eval': norm_ecmp_mlu_eval[L * 3:L * 4]})  # ,
        # df = pd.DataFrame({'traffic_r_l': norm_mlu_eval, 'flow_r_l': norm_ecmp_mlu_eval})#,
        # 将DataFrame存储为csv,index表示是否显示行名，default=True;
        # 在参数header和index中使用True或False指定是否指定header（列名，pandas.DataFrame的列）和index（行名，pandas.DataFrame的索引）。默认值为True。
        df.to_csv(path_plt_data + '/' + 'GENAT'+attach_word+'propXXX.csv', header=True, index=False,
    ...
    # path_reward_plt_data = os.path.join(root + '/result', 'plt_data')
    # import pandas as pd
    # if not os.path.exists("%s" % path_reward_plt_data):
    #     os.makedirs("%s" % path_reward_plt_data)
    # # 字典中的key值即为csv中列名
    # df = pd.DataFrame({'Sw-propXXX': nme_7})
    #
    # # 将DataFrame存储为csv,index表示是否显示行名，default=True;
    # # 在参数header和index中使用True或False指定是否指定header（列名，pandas.DataFrame的列）和index（行名，pandas.DataFrame的索引）。默认值为True。
    # df.to_csv(path_reward_plt_data + '/' + 'Sw-propXXX.csv', header=True, index=False, sep='\t',
...
def main():
    ...
    global attach_word
    attach_word_list = [[3,5,14,2,16],[3,5,14,2,16,6,18]]#[[14],[3,5,14]]#,[5],[]]
    for i_l in attach_word_list:
        RemoveNode = i_l
        attach_word = '_rmnd_DRL_TE_' + '_'.join(map(str, RemoveNode)) + '_'
        print(attach_word)
        ...
        if not os.path.exists("%s" % model_save_pkl) or not os.path.exists("%s" % env_save_pkl):
            ...
            model, env = model_train(env_save_pkl, model_save_pkl, link_num_ratio=0.15, flow_num_ratio=0.15, RemoveNode=RemoveNode)
        else:

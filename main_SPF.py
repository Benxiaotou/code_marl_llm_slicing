...
from utlis.custom_environment_spf import CustomEnv, eval_env_step#, env_init_eval#, env_init_real_TM_eval, env_init_simu_TM_eval,
...
def model_train(env_save_pkl, model_save_pkl, link_num_ratio, flow_num_ratio):
    ...
    env = CustomEnv(path_reward_save=path_reward_save,
                    ...
                    is_training=True)
    env.total_steps = 512*5 #504*1500  # 1500 episodes => 768000## 2000 episodes => 1024000
...
def model_eval(env, model):
    ...
    for k_episode in range(num_try):
        ...
        # 字典中的key值即为csv中列名
        L = int(len(time_array) / 7)
        df = pd.DataFrame({'SPF': time_array[L * 5:L * 6]})
        # 将DataFrame存储为csv,index表示是否显示行名，default=True;
        # 在参数header和index中使用True或False指定是否指定header（列名，pandas.DataFrame的列）和index（行名，pandas.DataFrame的索引）。默认值为True。
        df.to_csv(path_plt_data + '/' + 'SwitchL3_time_SPF.csv', header=True, index=False, sep='\t',

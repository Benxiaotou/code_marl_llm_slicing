...
class CustomMultiAgentEnv(RawMultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        for i in range(self.num_nodes):
            self.DG.nodes[i]['comp_capacity'] = 300 * 1.78 # 1000 * (2/4) # ！！ # ** 此处更改后，应删除相关的sample_save文件
            self.DG.nodes[i]['cach_capacity'] = 150 * 1.78 # 500 * (2/4) # ！！ # ** 此处更改后，应删除相关的sample_save文件
        ...
        for i in range(self.num_links):
            ...
            self.DG[s][d]['bandwidth'] = 10 * 1.78 # 33 * (2/4) # ！！ # ** # Gbps  此处更改后，应删除相关的sample_save文件
            self.DG[s][d]['allocated_bandw'] = 0  # 物理网络链路中已分配的带宽资源
            self.link_capacities = 10 * 1.78 # 33 * (2/4) # ！！ # ** # Gbps #重新修改初始的链路容量参数  此处更改后，应删除相关的sample_save文件
        ...
        self.observation_space = {self.agents[0]: Box(low=0, high=1.0, shape=[obs_k_one_shape, ], dtype=np.float32),
                                  ...
                                  # self.agents[3]: Box(low=0, high=1.0, shape=[obs_k_one_shape, ], dtype=np.float32),
                                  # self.agents[4]: Box(low=0, high=1.0, shape=[obs_k_one_shape, ], dtype=np.float32),
        ...
        self.action_space = {self.agents[0]: Box(low=-1, high=1.0, shape=[self.action_k_one_shape, ], dtype=np.float32),
                             ...
                             # self.agents[3]: Box(low=-1, high=1.0, shape=[self.action_k_one_shape, ], dtype=np.float32),
                             # self.agents[4]: Box(low=-1, high=1.0, shape=[self.action_k_one_shape, ], dtype=np.float32),
        ...
        # ！！ # **
        self.global_state = np.hstack((obs_k1,# obs_k2, obs_k3, obs_k4, obs_k5,
                                       obs_g))
    ...
    def step(self, action_dict):
        ...
        # ！！ # **
        reward_k1 = contribution_degree[0] * (V_k1 - V_k1_0) / (V_k1_0 + OBJ_EPSILON) * 10 * 3 if g_done == True else 0
        ...
        observations = {self.agents[0]: obs_k1,
                        # self.agents[1]: obs_k2,
                        # self.agents[2]: obs_k3,
                        # self.agents[2]: obs_k4,
                        # self.agents[2]: obs_k5,
                        self.agents[-1]: obs_g}
    ...
    def eval_env_step(self, action_dict):
        ...
        # reward_k2 = contribution_degree[1] * (V_k2 - V_k2_0) / (V_k2_0 + OBJ_EPSILON) * 10 * 4 if g_done == True else 0
        # reward_k3 = contribution_degree[2] * (V_k3 - V_k3_0) / (V_k3_0 + OBJ_EPSILON) * 10 * 4 if g_done == True else 0
        # reward_k4 = contribution_degree[2] * (V_k4 - V_k4_0) / (V_k4_0 + OBJ_EPSILON) * 10 * 4 if g_done == True else 0
        # reward_k5 = contribution_degree[2] * (V_k5 - V_k5_0) / (V_k5_0 + OBJ_EPSILON) * 10 * 4 if g_done == True else 0
...
class CustomIPPO_Agents(IPPO_Agents):
    """The custom implementation of IPPO agents.
    Args:
    ...
    def __init__(self,
        ...
        # 调用父类的构造函数
        super(CustomIPPO_Agents, self).__init__(config, envs)
        # 初始化自定义参数
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="configs_main_INDRL.yaml")
    ...
    save_attach_word = str(configs_dict['max_num_steps']) + '_' \
                       + configs_dict['topology_file'] + '_INDRL_kn-1_' # ！！
    ...
    #
    # Agents = CustomIPPO_Agents(config=configs, envs=envs, path_reward_save=path_reward_save)  # Create the Independent PPO agents.
    ...
    if configs.benchmark:
        ...
        marl_model_dir = 'dir_model_INDRL_kn-1'  # ！！ 主要更改它 # 为避免load错误模型，dir 存放一个.pth的model文件
        # marl_model_dir = 'dir_model_1'  # 主要更改它 # 为避免load错误模型，dir 存放一个.pth的model文件
        marl_model_name = save_attach_word + "ippo_model_save" + ".pth"
        ...
        if os.path.exists(path_marl_model) == False or os.path.exists(path_marl_env) == False:
            envs = make_envs(configs)  # Make the environment.
            Agents = CustomIPPO_Agents(config=configs, envs=envs)  # Create the Independent PPO agents.
            ...
            Agents = CustomIPPO_Agents(config=configs, envs=envs_save)  # Create the Independent PPO agents.

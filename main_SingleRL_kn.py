...
class CustomNetwork(nn.Module):
    ...
    def __init__(
            self,
            feature_dim: int,
            last_layer_dim_pi: int = 128,#32*5
            last_layer_dim_vf: int = 128,#32*5
    ):
        ...
        #     nn.Flatten(),
        #     nn.Linear(32*5, 64), ## max_dp_length=8时,32*6
        #     nn.ReLU(),
        # ).to(device)
...
class CustomActorCriticPolicy(ActorCriticPolicy):
    def __init__(
        ...
        # self.features_dim = self.observation_space
    ...
    #     # return ExtractFeaturesNetwork(self.features_dim)

    def _build_mlp_extractor(self) -> None:
        self.mlp_extractor = CustomNetwork(feature_dim=self.features_dim)
...
class CustomEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        for i in range(self.num_nodes):
            self.DG.nodes[i]['comp_capacity'] = 300*2.556 # 1000 * (3/4) # ！！ # ** 此处更改后，应删除相关的sample_save文件
            self.DG.nodes[i]['cach_capacity'] = 150*2.556 # 500 * (3/4) # ！！ # ** 此处更改后，应删除相关的sample_save文件
        ...
        for i in range(self.num_links):
            ...
            self.DG[s][d]['bandwidth'] = 10*2.556 # 33 * (3/4) # ！！ # ** # Gbps 此处更改后，应删除相关的sample_save文件
            self.DG[s][d]['allocated_bandw'] = 0  # 物理网络链路中已分配的带宽资源
            self.link_capacities = 10*2.556 # 33 * (3/4) # ！！ # ** # Gbps #重新修改初始的链路容量参数  此处更改后，应删除相关的sample_save文件
        ...
        self.K_num_existing_v_net = 1  # ！！ # **# 论文中的 K
        ...
        # ！！ # **
        self.observation_space = spaces.Box(low=0, high=1.0, shape=(self.K_num_existing_v_net * obs_k_one_shape
                                                                    + obs_g_one_shape,), dtype=np.float32)
        ...
        # ！！ # **
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(self.K_num_existing_v_net*self.action_k_one_shape
                                                                  + self.action_k_one_shape,), dtype=np.float32)
        ...
        # 用于收敛性分析和评价指标
        # ！！ # **
        self.dict_train_index = {'V': [], 'V_g': [],
                                 'V_k1': [],# 'V_k2': [], 'V_k3': [], 'V_k4': [], 'V_k5': [],
                                 'V_k1_0': [],# 'V_k2_0': [], 'V_k3_0': [], 'V_k4_0': [], 'V_k5_0': [],
                                 'X_k1': [],# 'X_k2': [], 'X_k3': [], 'X_k4': [], 'X_k5': [],
                                 'cost_k1': [],# 'cost_k2': [], 'cost_k3': [], 'cost_k4': [], 'cost_k5': [],
                                 'revenue_g': [],
                                 'revenue_k1': [],# 'revenue_k2': [], 'revenue_k3': [], 'revenue_k4': [], 'revenue_k5': []
                                 }
        ...
        # ！！ # **
        self.k1_net, self.g_net, self.DG = self.sample_dict[self.sample_idx]
        # self.k1_net, self.k2_net, self.k3_net, self.k4_net, self.k5_net, self.g_net, self.DG = self.sample_dict[self.sample_idx]
        ...
        # idx_k4_overlap_nodes, idx_k4_overlap_links, _ = self._get_overlap_node_link(k_net=self.k4_net, g_net=self.g_net)
        # idx_k5_overlap_nodes, idx_k5_overlap_links, _ = self._get_overlap_node_link(k_net=self.k5_net, g_net=self.g_net)
        ...
        # idx_k4_overlap_nodes = sorted(idx_k4_overlap_nodes)  # ！ 按照从小到大的顺序排列
        # idx_k5_overlap_nodes = sorted(idx_k5_overlap_nodes)  # ！ 按照从小到大的顺序排列
        ...
        # idx_k4_overlap_links = sorted(idx_k4_overlap_links)  # ！ 按照从小到大的顺序排列
        # idx_k5_overlap_links = sorted(idx_k5_overlap_links)  # ！ 按照从小到大的顺序排列
        ...
        # obs_k4 = self._get_v_net_observations(self.k4_net, idx_k4_overlap_nodes, idx_k4_overlap_links, is_zero_padding)
        # obs_k5 = self._get_v_net_observations(self.k5_net, idx_k5_overlap_nodes, idx_k5_overlap_links, is_zero_padding)

        obs_g = self._get_g_net_observations(is_zero_padding)  # ！ 采用补零方案1

        # ！！ # **
        self.observations = {self.agents[0]: obs_k1,
                             # self.agents[1]: obs_k2,
                             # self.agents[2]: obs_k3,
                             # self.agents[3]: obs_k4,
                             # self.agents[4]: obs_k5,
                             self.agents[-1]: obs_g}

        # ！！ # **
        self.state = np.hstack((obs_k1,# obs_k2, obs_k3, obs_k4, obs_k5,
                                       obs_g))
    ...
    def step(self, action):  # 输出层使用 tanh，即输出为 -1～1 之间的浮点数; 经过缩放，对应到实际动作
        ...
        # ！！ # **
        action_dict['agent_k1'] = action[:self.action_k_one_shape]
        # action_dict['agent_k2'] = action[self.action_k_one_shape:self.action_k_one_shape * 2]
        # action_dict['agent_k3'] = action[self.action_k_one_shape * 2:self.action_k_one_shape * 3]
        # action_dict['agent_k4'] = action[self.action_k_one_shape * 3:self.action_k_one_shape * 4]
        # action_dict['agent_k5'] = action[self.action_k_one_shape * 4:self.action_k_one_shape * 5]

        action_dict['agent_g'] = action[self.action_k_one_shape*self.K_num_existing_v_net:]
        ...
        # #####################################################################
        # ## 代价计算
        # # 输出 idx_k4_overlap_nodes和idx_k4_overlap_links
        # idx_k4_overlap_nodes, idx_k4_overlap_links, sd_k_overlap_links = self._get_overlap_node_link(k_net=self.k4_net,
        #                                                                                              g_net=self.g_net)
        # idx_k4_overlap_nodes = sorted(idx_k4_overlap_nodes)  # ！ 按照从小到大的顺序排列
        # idx_k4_overlap_links = sorted(idx_k4_overlap_links)  # ！ 按照从小到大的顺序排列
        # # idx_k4_overlap_nodes = [1, 2, 3, 9, 10]  # 按照idx排列，并于action_k4_node一一对应。
        # action_k4 = actions['agent_k4']
        # action_k4_node = action_k4[:self.action_k_node_control]  # 节点控制
        # action_k4_node, control_scale = self._get_scale_repeat_action(action_k4_node, idx_k4_overlap_nodes)
        # old_demand_n_k4 = self._get_net_node_demand(v_net=self.k4_net)
        # cost_n_k4, self.k4_net, assigned_comp_k4, assigned_cach_k4 = self._get_k_node_cost(k_net=self.k4_net,
        #                                                                                    idx_overlap_nodes=idx_k4_overlap_nodes,
        #                                                                                    action_k_node=action_k4_node)
        #
        # action_k4_link = action_k4[self.action_k_node_control:]  # 链路控制
        # action_k4_link, control_scale = self._get_scale_repeat_action(action_k4_link, idx_k4_overlap_links)
        #
        # k4_tm_idx = 0 if self.fixed_tm_idx == True else self.sample_idx
        # cost_l_k4, self.k4_net, assigned_bandw_k4, demand_l_k4, old_demand_l_k4 = \
        #     self._get_k_link_cost(k_net=self.k4_net, idx_overlap_links=idx_k4_overlap_links,
        #                           action_k_link=action_k4_link, tm_idx=k4_tm_idx)
        # beta_n = 0.01 / 2
        # beta_e = 1 / 5
        # cost_k4 = beta_n * cost_n_k4 + beta_e * cost_l_k4
        #
        # ## 收益计算
        # w_n = 0.01
        # w_l = 10
        # old_revenue_k4 = w_n * old_demand_n_k4 + w_l * old_demand_l_k4  # 在新生成业务网络之前，第k个已有网络所获得的来自任务服务的收益
        # demand_n_k4 = self._get_net_node_demand(v_net=self.k4_net)
        # revenue_k4 = w_n * demand_n_k4 + w_l * demand_l_k4  # 在新生成业务网络之后，第k个已有网络所获得的来自任务服务的收益

        # ############################ k5 ##################################### # ！！ # **
        # #####################################################################
        # ## 代价计算
        # # 输出 idx_k5_overlap_nodes和idx_k5_overlap_links
        # idx_k5_overlap_nodes, idx_k5_overlap_links, sd_k_overlap_links = self._get_overlap_node_link(k_net=self.k5_net,
        #                                                                                              g_net=self.g_net)
        # idx_k5_overlap_nodes = sorted(idx_k5_overlap_nodes)  # ！ 按照从小到大的顺序排列
        # idx_k5_overlap_links = sorted(idx_k5_overlap_links)  # ！ 按照从小到大的顺序排列
        # # idx_k5_overlap_nodes = [1, 2, 3, 9, 10]  # 按照idx排列，并于action_k5_node一一对应。
        # action_k5 = actions['agent_k5']
        # action_k5_node = action_k5[:self.action_k_node_control]  # 节点控制
        # action_k5_node, control_scale = self._get_scale_repeat_action(action_k5_node, idx_k5_overlap_nodes)
        # old_demand_n_k5 = self._get_net_node_demand(v_net=self.k5_net)
        # cost_n_k5, self.k5_net, assigned_comp_k5, assigned_cach_k5 = self._get_k_node_cost(k_net=self.k5_net,
        #                                                                                    idx_overlap_nodes=idx_k5_overlap_nodes,
        #                                                                                    action_k_node=action_k5_node)
        #
        # action_k5_link = action_k5[self.action_k_node_control:]  # 链路控制
        # action_k5_link, control_scale = self._get_scale_repeat_action(action_k5_link, idx_k5_overlap_links)
        #
        # k5_tm_idx = 0 if self.fixed_tm_idx == True else self.sample_idx
        # cost_l_k5, self.k5_net, assigned_bandw_k5, demand_l_k5, old_demand_l_k5 = \
        #     self._get_k_link_cost(k_net=self.k5_net, idx_overlap_links=idx_k5_overlap_links,
        #                           action_k_link=action_k5_link, tm_idx=k5_tm_idx)
        # beta_n = 0.01 / 2
        # beta_e = 1 / 5
        # cost_k5 = beta_n * cost_n_k5 + beta_e * cost_l_k5
        #
        # ## 收益计算
        # w_n = 0.01
        # w_l = 10
        # old_revenue_k5 = w_n * old_demand_n_k5 + w_l * old_demand_l_k5  # 在新生成业务网络之前，第k个已有网络所获得的来自任务服务的收益
        # demand_n_k5 = self._get_net_node_demand(v_net=self.k5_net)
        # revenue_k5 = w_n * demand_n_k5 + w_l * demand_l_k5  # 在新生成业务网络之后，第k个已有网络所获得的来自任务服务的收益
        #
        # #####################################################################
        # #####################################################################
        ...
        # assigned_resoure['k4_net'] = (assigned_comp_k4, assigned_cach_k4, assigned_bandw_k4)
        # assigned_resoure['k5_net'] = (assigned_comp_k5, assigned_cach_k5, assigned_bandw_k5)
        ...
        if g_done == True:
            ...
            # X_k4 = revenue_g * contribution_degree[3]
            # X_k5 = revenue_g * contribution_degree[4]
        ...
        # V_k4_0 = old_revenue_k4
        # V_k4 = revenue_k4 - cost_k4 + X_k4
        # V_k5_0 = old_revenue_k5
        # V_k5 = revenue_k5 - cost_k5 + X_k5

        V_g_0 = 0

        # ！！ # **
        V_g = revenue_g - X_k1# - X_k2 - X_k3 - X_k4 - X_k5
        ...
        # reward_k4 = contribution_degree[3] * (V_k4 - V_k4_0) / (V_k4_0 + OBJ_EPSILON) * 10 * 4 if g_done == True else 0
        # reward_k5 = contribution_degree[4] * (V_k5 - V_k5_0) / (V_k5_0 + OBJ_EPSILON) * 10 * 4 if g_done == True else 0

        reward_g = contribution_degree[-1] * (V_g - V_g_0) / 20 * 1.5 if g_done == True else 0
        # clip
        ...
        # reward_k4 = np.clip(reward_k4, -5, 5)
        # reward_k5 = np.clip(reward_k5, -5, 5)
        ...
        # ！！ # **
        reward = (reward_k1 + reward_g) / 2
        # reward = (reward_k1 + reward_k2 + reward_k3 + reward_k4 + reward_k5 + reward_g) / 6
        ...
        # self.reward_k4_list.append(reward_k4)
        # self.reward_k5_list.append(reward_k5)
        ...
        if self.timestep + 1 == self.max_num_steps:
            ...
            with open(self.path_reward_save + '/' + self.save_attach_word + 'rewards_marl_env.pkl',
                ...
                # ！！ # **
                pickle.dump((self.reward_k1_list,
                             # self.reward_k2_list,
                             # self.reward_k3_list,
                             # self.reward_k4_list,
                             # self.reward_k5_list,
        ...
        # ！！ # **
        self.dict_train_index['V'].append(V_k1 + V_g) # ！！ # <1> 总效益 V
        # self.dict_train_index['V'].append(V_k1 + V_k2 + V_k3 + V_k4 + V_k5 + V_g) # <1> 总效益 V
        ...
        # self.dict_train_index['V_k4'].append(V_k4)  # <1> K4 效益 V_k4
        # self.dict_train_index['V_k5'].append(V_k5)  # <1> K5 效益 V_k5
        ...
        # self.dict_train_index['V_k4_0'].append(V_k4_0)
        # self.dict_train_index['V_k5_0'].append(V_k5_0)
        self.dict_train_index['X_k1'].append(X_k1)
        # self.dict_train_index['X_k2'].append(X_k2)
        # self.dict_train_index['X_k3'].append(X_k3)
        # self.dict_train_index['X_k4'].append(X_k4)
        # self.dict_train_index['X_k5'].append(X_k5)
        ...
        # self.dict_train_index['cost_k4'].append(cost_k4)
        # self.dict_train_index['cost_k5'].append(cost_k5)
        ...
        # self.dict_train_index['revenue_k4'].append(revenue_k4)
        # self.dict_train_index['revenue_k5'].append(revenue_k5)
        ...
        # ！！ # **
        observations = {self.agents[0]: obs_k1,
                        # self.agents[1]: obs_k2,
                        # self.agents[2]: obs_k3,
                        # self.agents[3]: obs_k4,
                        # self.agents[4]: obs_k5,
        ...
        # ！！ # **
        self.state = np.hstack((obs_k1, obs_g))
        # self.state = np.hstack((obs_k1, obs_k2, obs_k3, obs_k4, obs_k5, obs_g))
    ...
    def _get_contribution_degree(self, assigned_resoure, unassigned_resoure):
        ...
        # assigned_comp_k4, assigned_cach_k4, assigned_bandw_k4 = assigned_resoure['k4_net']
        # assigned_comp_k5, assigned_cach_k5, assigned_bandw_k5 = assigned_resoure['k5_net']
        ...
        # cd_k4_to_g_node_comp, cd_k4_to_g_node_cach, cd_k4_to_g_link \
        #     = self._get_cd_k_to_g_node_link(k_net=self.k4_net, g_net=self.g_net,
        #                                     assigned_comp_k=assigned_comp_k4, assigned_cach_k=assigned_cach_k4,
        #                                     assigned_bandw_k=assigned_bandw_k4)
        # cd_k5_to_g_node_comp, cd_k5_to_g_node_cach, cd_k5_to_g_link \
        #     = self._get_cd_k_to_g_node_link(k_net=self.k5_net, g_net=self.g_net,
        #                                     assigned_comp_k=assigned_comp_k5, assigned_cach_k=assigned_cach_k5,
        #                                     assigned_bandw_k=assigned_bandw_k5)
        ############################################################################
        ...
        # ！！ # **
        cd_vstack_comp = np.vstack((cd_k1_to_g_node_comp,
                                    # cd_k2_to_g_node_comp,
                                    # cd_k3_to_g_node_comp,
                                    # cd_k4_to_g_node_comp,
                                    # cd_k5_to_g_node_comp,
        ...
        # ！！ # **
        cd_vstack_cach = np.vstack((cd_k1_to_g_node_cach,
                                    # cd_k2_to_g_node_cach,
                                    # cd_k3_to_g_node_cach,
                                    # cd_k4_to_g_node_cach,
                                    # cd_k5_to_g_node_cach,
        ...
        # ！！ # **
        cd_vstack_link = np.vstack((cd_k1_to_g_link,
                                    # cd_k2_to_g_link,
                                    # cd_k3_to_g_link,
                                    # cd_k4_to_g_link,
                                    # cd_k5_to_g_link,
                                    p_l_w_factor * cd_p_to_g_link))  # 堆叠
...
def eval_env_step(self, action):
    ...
    action_g = actions['agent_g']  # [0.5, 0.5, 0.5]  # 分别代表计算、缓存、带宽资源的调整比例
    self.g_net, demand_n_g, demand_l_g, g_done = self._get_g_adjustment(g_net=self.g_net, action_g=action_g,
                                                                        assigned_resoure=assigned_resoure,
                                                                        tm_idx=g_tm_idx)
    if g_done == True:
        # g 的收益计算
        revenue_g = w_n * demand_n_g + w_l * demand_l_g
    ...
    # ！！ # **
    rewards = {self.agents[0]: reward_k1,
               # self.agents[1]: reward_k2,
               # self.agents[2]: reward_k3,
               # self.agents[3]: reward_k4,
               # self.agents[4]: reward_k5,
               self.agents[-1]: reward_g}
    ...
    # ！！ # **
    self.dict_eval_index['V'].append(V_k1 + V_g)  # <1> 总效益 V
    # self.dict_eval_index['V'].append(V_k1 + V_k2 + V_k3 + V_k4 + V_k5 + V_g)  # <1> 总效益 V
    ...
    # self.dict_eval_index['V_k4'].append(V_k4)  # <1> K3 效益 V_k4
    # self.dict_eval_index['V_k5'].append(V_k5)  # <1> K3 效益 V_k5
    ...
    # self.dict_eval_index['V_k4_0'].append(V_k4_0)
    # self.dict_eval_index['V_k5_0'].append(V_k5_0)
    self.dict_eval_index['X_k1'].append(X_k1)
    # self.dict_eval_index['X_k2'].append(X_k2)
    # self.dict_eval_index['X_k3'].append(X_k3)
    # self.dict_eval_index['X_k4'].append(X_k4)
    # self.dict_eval_index['X_k5'].append(X_k5)
    ...
    # self.dict_eval_index['cost_k4'].append(cost_k4)
    # self.dict_eval_index['cost_k5'].append(cost_k5)
    ...
    # self.dict_eval_index['revenue_k4'].append(revenue_k4)
    # self.dict_eval_index['revenue_k5'].append(revenue_k5)
...
def model_train(env_save_pkl, model_save_pkl):

    logging.info("Begin medel_train")
    setup_seed(seed=1)

    env = CustomEnv(config=configs)
    # env.max_num_steps = configs.max_num_steps #504*1500  # 1500 episodes => 768000## 2000 episodes => 1024000
    ...
    model.learn(total_timesteps=configs.max_num_steps)
...
def model_eval(env, model):
    ...
    while episode_count < n_episodes:
        ...
        obs, reward, dones, info = eval_env_step(env, action)
        print('eval_step_count = ' + str(eval_step_count))

        eval_step_count += 1
        if eval_step_count % env.eval_max_step == 0:
            episode_count += 1
            with open(env.path_reward_save + '/' + env.save_attach_word + 'eval_marl_env.pkl',
                      'rb') as handle:

                # ！！ # **
                reward_k1_list, reward_g_list, g_done_list, dict_eval_index = pickle.load(handle)
                # reward_k1_list, reward_k2_list, reward_k3_list, reward_k4_list, reward_k5_list,\
                # reward_g_list, g_done_list, dict_eval_index = pickle.load(handle)
            ...
            os.remove(env.path_reward_save + '/' + env.save_attach_word + 'eval_marl_env.pkl')
        else:
            pass

    # with open(env.path_reward_save + '/' + env.save_attach_word + 'dict_train_index_marl_env.pkl',
    #           'rb') as handle:
    ...
    # path_plt_data = self.path_reward_save.replace('reward_save', 'plt_data')
    # if not os.path.exists("%s" % path_plt_data):
    #     os.makedirs("%s" % path_plt_data)
    # # 字典中的key值即为csv中列名
    # df = pd.DataFrame({'cg-5_V': r_mean_L})
    # # 将DataFrame存储为csv,index表示是否显示行名，default=True;
    # # 在参数header和index中使用True或False指定是否指定header（列名，pandas.DataFrame的列）和index（行名，pandas.DataFrame的索引）。默认值为True。
    # df.to_csv(path_plt_data + '/' + 'cg-5_V.csv', header=True, index=False, sep='\t',
    #           float_format='%.5f')  # index为true会多写一列index,分隔符默认为逗号
...
def main():
    ...
    model_save_pkl = os.path.join(path_model_save, configs.save_attach_word + 'model_save.pkl')
    env_save_pkl = os.path.join(path_env_save, configs.save_attach_word + 'env_save.pkl')

    if not os.path.exists("%s" % model_save_pkl) or not os.path.exists("%s" % env_save_pkl):
        logging.info("———————生成env和DRL_model—————")
        model_train(env_save_pkl, model_save_pkl)
    else:
...
if __name__ == "__main__":
    ...
    parser = parse_args()
    configs_dict = get_configs(file_dir="configs_main_SingleRL.yaml")
    ...
    save_attach_word = str(configs_dict['max_num_steps']) + '_' \
                       + configs_dict['topology_file'] + '_SingleRL_kn-1' + '_' #！！

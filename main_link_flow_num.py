...
from utlis.custom_environment import CustomEnv, eval_env_step#, env_init_eval#, env_init_real_TM_eval, env_init_simu_TM_eval,
...
def model_eval(env, model):
    ...
    # setup_seed(SEED)
    # if config.topology_file == 'Abilene' or config.topology_file == 'EGANT':
    #     env = env_init_real_TM_eval(env)
    # else:
    #     env = env_init_simu_TM_eval(env)
    # env = env_init_eval(env)#考虑’env_init_eval(env)‘
...
def main():
    ...
    link_num_ratio_list = [0.05, 0.10, 0.15, 0.20, 0.25]
    for i_l in link_num_ratio_list:
        link_num_ratio = i_l
        flow_num_ratio = 0.15 # 固定该值
        attach_word = '_LinkNum_'+str(link_num_ratio)+'_FlowNum_'+str(flow_num_ratio)+'_'#'_'
        ...
        if not os.path.exists("%s" % model_save_pkl) or not os.path.exists("%s" % env_save_pkl):
            ...
            model_train(env_save_pkl, model_save_pkl, link_num_ratio=link_num_ratio, flow_num_ratio=flow_num_ratio)
        else:
    ...
    flow_num_ratio_list = [0.05, 0.10, 0.15, 0.20, 0.25]
    for i_f in flow_num_ratio_list:
        link_num_ratio = 0.15  # 固定该值
        flow_num_ratio = i_f
        attach_word = '_LinkNum_' + str(link_num_ratio) + '_FlowNum_' + str(flow_num_ratio) + '_'  # '_'
        model_save_pkl = os.path.join(path_model_save, config.topology_file + attach_word + 'model_save.pkl')
        env_save_pkl = os.path.join(path_env_save_save, config.topology_file + attach_word + 'env_save.pkl')
